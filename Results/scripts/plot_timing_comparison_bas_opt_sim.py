...
impl  = dfs["simd c++"]["implementation"].iloc[0]
batch = int(dfs["simd c++"]["batch"].iloc[0])

N_values = dfs["simd c++"]["channels"].values  # channels = BASE_C*N = N
...
# Axes labels & title
ax.set_title(
    f"Baseline c++, Optimized c++ and Vectorized c++ code timing Comparison (batch size {batch})",
...
# Save
OUT = "timing_comparison_bas_opt_sim.png"
plt.tight_layout()
