...
def count_equi_rms_norm(*, B: int, T: int, C: int) -> dict:
    ...
    Per token (N = B*T):
      ...
      - scale + optional weight multiply: C channels × 16 blades × 2
    """
    N = B * T
    ip_flops   = N * C * (8 + 7)       # 8 squares, 7 adds per channel
    norm_flops = N * (1 + 2)           # mean div + sqrt + rcp
    scale_flops = N * C * 16 * 2       # multiply by (scale * weight)
...
def count_attention(
    *,
    ...
    # Phase 1
    norm_flops = 2 * B * H * max(Tq, Tk) * Cqk * 7
...
if __name__ == "__main__":
    ...
    configs = [
        ...
        ("full_forward",      dict(B=1, T=64, C=8, H=4)),
    ]
