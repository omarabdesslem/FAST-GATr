...
static torch::Tensor _fused_sdpa_ipa_daa(
    ...
    // Normalize the trivectors of q and k once up front
    auto qN = torch::empty({B, H, Tq, Cqk, 4}, q_mv.options());
    auto kN = torch::empty({B, H, Tk, Cqk, 4}, k_mv.options());
    float* qNp = qN.data_ptr<float>();
    float* kNp = kN.data_ptr<float>();

    auto fill_norm = [&](const float* src, float* dst, int64_t T) {
        const int64_t N = B * H * T * Cqk;
        for (int64_t n = 0; n < N; ++n) {
            const float* tri = src + n * 16 + 11;
    ...
    for (int64_t b = 0; b < B; ++b) {
        for (int64_t h = 0; h < H; ++h) {
            const float* w_ipa_h = wip + h * Cqk;
            const float* w_daa_h = wdp + h * Cqk;

            for (int64_t i = 0; i < Tq; ++i) {
                const float* qi_mv = qp  + (((b * H + h) * Tq + i) * Cqk) * 16;
                const float* qi_N  = qNp + (((b * H + h) * Tq + i) * Cqk) * 4;

                float max_score = NEG_INF;

                for (int64_t j = 0; j < Tk; ++j) {
                    const float* kj_mv = kp  + (((b * H + h) * Tk + j) * Cqk) * 16;
                    const float* kj_N  = kNp + (((b * H + h) * Tk + j) * Cqk) * 4;

                    float dot = 0.0f;
                    for (int64_t c = 0; c < Cqk; ++c) {
                        const float* qc = qi_mv + c * 16;
                        const float* kc = kj_mv + c * 16;

                        // dot over blades 0..10 and 15
                        const float ipa =
                              qc[0]  * kc[0]
                            + qc[2]  * kc[2]
                            + qc[3]  * kc[3]
                        ...
                        // DAA: q_daa * k_daa worked out by hand from the (4,4,5) basis. Both vectors live in R^5, but most products collapse and we're left with the 5 terms below.
                        const float* qn = qi_N + c * 4;
                        const float* kn = kj_N + c * 4;
                        const float qn3 = qn[3];
                        const float kn3 = kn[3];
                        const float qsum = qn[0]*qn[0] + qn[1]*qn[1] + qn[2]*qn[2];
                        const float ksum = kn[0]*kn[0] + kn[1]*kn[1] + kn[2]*kn[2];
                        const float cross = qn[0]*kn[0] + qn[1]*kn[1] + qn[2]*kn[2];
                        const float daa =
                              -qsum * (kn3 * kn3)
                            - (qn3 * qn3) * ksum
                            + 2.0f * qn3 * kn3 * cross;

                        dot += w_ipa_h[c] * ipa + w_daa_h[c] * daa;
                    }
                ...
                // Dv = Cv * 16. Unroll the inner 16
                // leave the channel loop rolled.
                for (int64_t j = 0; j < Tk; ++j) {
                    ...
                    int64_t d = 0;
                    for (; d + 16 <= Dv; d += 16) {
                        o_row[d + 0]  += a * v_row[d + 0];
                        o_row[d + 1]  += a * v_row[d + 1];
                        o_row[d + 2]  += a * v_row[d + 2];
                        o_row[d + 3]  += a * v_row[d + 3];
                        o_row[d + 4]  += a * v_row[d + 4];
                        o_row[d + 5]  += a * v_row[d + 5];
                        o_row[d + 6]  += a * v_row[d + 6];
                        o_row[d + 7]  += a * v_row[d + 7];
                        o_row[d + 8]  += a * v_row[d + 8];
                        o_row[d + 9]  += a * v_row[d + 9];
                        o_row[d + 10] += a * v_row[d + 10];
                        o_row[d + 11] += a * v_row[d + 11];
                        o_row[d + 12] += a * v_row[d + 12];
                        o_row[d + 13] += a * v_row[d + 13];
                        o_row[d + 14] += a * v_row[d + 14];
                        o_row[d + 15] += a * v_row[d + 15];
                    }
                    // in case Dv isn't a multiple of 16
                    for (; d < Dv; ++d) o_row[d] += a * v_row[d];
                }
            }
        }
    }
